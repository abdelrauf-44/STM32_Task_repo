...
void reset_all_led(void){
	...
	for (int i=0; i<24*LED_Number; i++) PWM_data[i] = WS2812B_LOW;
...
void Set_LED (int LED_Num, int Red, int Green, int Blue)
	{
	    ...
	    {
	    	PWM_data[24*LED_Num + 7 - i] = ((Green >> i) & 1) + 1;
	    }
	    for (int i=7; i>=0; i--) // Set the second 8 out of 24 to red
	    {
	    	PWM_data[24*LED_Num + 15 - i] = ((Red >> i) & 1) + 1;
	    }
	    for (int i=7; i>=0; i--) // Set the third 8 out of 24 to blue
	    {
	    	PWM_data[24*LED_Num + 23 - i] = ((Blue >> i) & 1) + 1;
	    }
	}

void WS2812B_SEND(void){

	HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*) PWM_data, 24*LED_Number);
